...
export const MarketView: React.FC<MarketViewProps> = ({
  ...
  const locationOptions = ['Semua Wilayah', 'Brebes', 'Bandung', 'Malang', 'Garut'];
  ...
  const filteredListings = farmerListings.filter((f) => {
    ...
    const matchesLocation = selectedLocation === 'Semua Wilayah' || f.location.includes(selectedLocation);
    // Dummy date matching for MVP
    const matchesDate = selectedDate === 'Semua Waktu' || f.date !== ''; 
    ...
    <div className="px-5 pt-3 pb-28 space-y-4 animate-in fade-in duration-200">
      ...
      <div className="flex gap-2">
        <select 
          ...
          className="flex-1 bg-white border border-gray-200 rounded-xl px-3 py-2 text-xs font-medium text-gray-700 focus:outline-none focus:border-[#2A4736]"
        >
